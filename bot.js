...
cron.schedule('30 0 * * * *', () => {
  ...
  https.get(Config.URL, (res) => {
    ...
    res.on('end', () => {
      const ninzuu = datas.split(/支援者合計|受付終了/g)[1].split('<dd>')[1].split('</dd>')[0];
      ...
      if(datas.indexOf("<dd>受付終了</dd>") == -1){
        if(ninzuu != text){
          ...
          console.log(syuuryoubi);
          const ato = Number(mokuhyou.replace(/,|円/g,""))-Number(goukei.replace(/,|円/g,""));
          console.log(ato);
          const embed = new MessageEmbed()
          .setColor('#0099ff')
          .setTitle(`支援者が ${ninzuu.split('人')[0]-text.split('人')[0]}人 増えた!!`)
        ...
        if(text != '受付終了'){
          fs.writeFile("text.txt", '受付終了', (err) => {if (err) throw err;console.log('バックアップ完了');});
          const embed = new MessageEmbed()
          .setColor('#0099ff')
          .setTitle('募金は終了しました')
          .setDescription(title)
          ...
          client.guilds.cache.get(Config.serverID).channels.cache.get(Config.channelID).send({ embeds: [embed] , components: [new MessageActionRow().addComponents(new MessageButton().setURL(Config.URL).setLabel(`URL`).setStyle(`LINK`))]});
        }
      }
    });
  });
    ...
    https.get(Config.URL, (res) => {
      ...
      res.on('end', () => {
        ...
        if(datas.indexOf("<dd>受付終了</dd>") == -1){
          ...
          const description = datas.split(/受付終了|この事業で選択可能なお礼品/g)[1].split(/<div class="cf-Article__body">/)[1].split('</div><!-- /.cf-Article__body -->')[0].replace(/<br .>/g,'');
          const embed = new MessageEmbed()
          .setColor('#0099ff')
          .setTitle(title)
          .setDescription(description)
          .addFields(
            ...
            { name: '目標金額', value: mokuhyou, inline: true },
            { name: '支援者合計', value: ninzuu, inline: true },
            { name: '受付終了日', value: '(募金は終了しました)', inline: true },
            { name: '達成率', value: `${tasseiritu.toLocaleString()}%`, inline: true },
            { name: '\u200B', value: '\u200B', inline: true }
          )
          interaction.reply({ embeds: [embed] , components: [new MessageActionRow().addComponents(new MessageButton().setURL(Config.URL).setLabel(`URL`).setStyle(`LINK`))]});
        }
      });
